...
## 项目简介
本项目是一个模块化`python`爬虫，用于采集网易新闻首页动态记载新闻的标题信息，并持久化存储到`MYSQL`数据库中。项目通过`API`接口分析使用`request`库请求获取`jsonp`数据，工程化设计延用豆瓣项目，将配置管理，数据抓取，数据存储解耦为独立板块。
...
2. 数据库存储类型和`spider`里返回的不同导致报错。通过统一数据类型为字典解决

3. `main.py`文件调用`spider.py`里的函数未传参导致错误。通过添加参数解决
